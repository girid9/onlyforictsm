...
const Revision = () => {
  ...
  const modes = [
    { id: "srs", title: "Spaced Repetition", description: "Questions you got wrong resurface after 1, 3, and 7 days.", icon: CalendarClock, color: "text-primary", bgColor: "bg-primary/10", stat: `${srsDue.length} due now`, disabled: srsDue.length === 0 },
    { id: "wrong", title: "Wrong Questions Only", description: "Re-practice all questions you got wrong.", icon: RotateCcw, color: "text-destructive", bgColor: "bg-destructive/10", stat: `${wrongCount} questions`, disabled: wrongCount === 0 },
    { id: "hard", title: "Hard Topics Only", description: "Focus on topics where your accuracy is below 50%.", icon: Brain, color: "text-warning", bgColor: "bg-warning/10", stat: `${hardTopicCount} topics`, disabled: hardTopicCount === 0 },
    { id: "fast20", title: "Fast 20 Challenge", description: "Random 20 questions with a 10-minute timer.", icon: Zap, color: "text-success", bgColor: "bg-success/10", stat: `${Math.min(20, totalQuestions)} questions`, disabled: totalQuestions === 0 },
  ];
  ...
  return (
    <div className="p-4 md:p-8 max-w-lg mx-auto min-h-screen">
      <h1 className="text-2xl font-bold text-foreground mb-1">Smart Revision</h1>
      <p className="text-sm text-muted-foreground mb-6">Choose a revision mode to strengthen your weak areas</p>
